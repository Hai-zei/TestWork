...
class Game:
    def __init__(self):
        ...
        self.obstacle_spawn_rate = 0.008  # 降低障碍物生成速率
        self.last_obstacle_x = SCREEN_WIDTH  # 记录上一个障碍物的 x 坐标
    ...
    def update(self):
        ...
        if random.random() < self.obstacle_spawn_rate and not self.game_over:
            if self.last_obstacle_x - SCREEN_WIDTH > -self.player.rect.width:  # 确保障碍物之间有足够距离
                ...
                self.last_obstacle_x = obstacle.rect.x  # 更新最后一个障碍物的 x 坐标

        # 碰撞检测：仅检测有颜色部分
        for obstacle in self.obstacles:
            if pygame.sprite.collide_mask(self.player, obstacle):  # 使用精确的像素掩码碰撞检测
                self.game_over = True
                break
